...
func main() {
	eq := [][]string{}
	eq = append(eq, []string{"a", "b"})
	eq = append(eq, []string{"b", "c"})
	val := []float64{2.0, 3.0}
	q := [][]string{}
	q = append(q, []string{"a", "c"})
	q = append(q, []string{"b", "a"})
	q = append(q, []string{"a", "e"})
	q = append(q, []string{"a", "a"})
	q = append(q, []string{"x", "x"})
