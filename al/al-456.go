...
* 遍历 nums 的时候，当前 num 如果小于栈顶，那么直接压栈，因为表明当前 num 往后的数(即栈顶) 比它大，不可能构成 132 子序列，
*
* 如果 num 比栈顶大，说明有可能构成 132 子序列，这时需要判断下当前 num 和他的 min 是否相等，相等就跳过，
* 如果不相等则需要将栈顶和 min 做个比较，如果栈顶大于 min 说明 132 成立，返回 true，如果栈顶小于 min，那就成了 231 不满足，
* 需要出栈，看看往后有没有能满足 132 的元素，因为栈内是单调递增的，所以后面的数都要比当前的栈顶大。
*
